...
###Gene abundance
cd Gene
SAF=./Abundance/final.contigs.genes.fna.saf
mkdir sam sort_bam flagstat abundance
bwa mem -M -Y -t 8 -o sam/${SampleID}.sam ${GeneCatalog}/cdhit_cluster/final.contigs.genes.fna ${CleanData}/${SampleID}.clean_R1.fastq.gz ${CleanData}/${SampleID}.clean_R1.fastq.gz
...
#merge all samples
paste -d '\t' abundance/*.fpkm.txt > abundance/All_gene_abundance.fpkm.txt
